...
jQuery(document).ready(function () {
	...
	jQuery.lazyLoadXT.onshow.addClass='lazy-hidden a3-portfolio-lazy-hidden';
		...
		$win.bind("load", function() {
			var checkLoadAllImages = setInterval( function() {
				...
				if ( jQuery("img.a3-portfolio-thumb-lazy").length < 1 ) {
					...
					jQuery('.a3-portfolio-box-content').masonry();
				}
			}, 1000 );
		});
	}
	...
	jQuery(document).on( 'click', '.pg_grid_content',function(){
		...
		if(pdcc.width() <= 586 ){
			var pdvScrollTop = current_card_activated.offset().top-(header_offset+20)+(current_card_activated.height()+20);
	...
	jQuery(document).on("click tap", ".a3-portfolio-navigation-mobile .a3-portfolio-icon-close", function(){
		...
		portfolio_container.find( ".a3-portfolio-menus-container" ).slideUp( "fast", function() {
			portfolio_container.find( '.a3-portfolio-navigation-mobile-icon' ).removeClass('a3-portfolio-icon-close').addClass('a3-portfolio-icon-list');
		});
	});
	jQuery(document).on("click tap", ".a3-portfolio-navigation-mobile .a3-portfolio-icon-list", function(){
		...
		portfolio_container.find( ".a3-portfolio-menus-container" ).slideDown( "fast", function() {
			portfolio_container.find( '.a3-portfolio-navigation-mobile-icon' ).removeClass('a3-portfolio-icon-list').addClass('a3-portfolio-icon-close');
		});
	});
	...
	jQuery(document).on("click", ".a3-portfolio-menus-container li a", function(){
		...
		if ( ! a3_portfolio_script_params.have_filters_script ) {
			...
			portfolio_boxes.find('.a3-portfolio-item').not(selector).hide();
			portfolio_boxes.find('.a3-portfolio-item'+selector).show();
			portfolio_boxes.masonry();
		}
		return false;
	});
				...
				if (pdv.length) {
					...
					setTimeout(function() {
						...
						if (force_scrolltotop) {
							a3_portfolio_openDetailView(portfolio_boxes,thumb,fade);
						} else {
							setTimeout(function () {
								a3_portfolio_openDetailView(portfolio_boxes,thumb,fade);
							},scrollspeed)
					...
					if (force_scrolltotop) {
						a3_portfolio_openDetailView(portfolio_boxes,thumb);
					} else {
						setTimeout(function () {
							a3_portfolio_openDetailView(portfolio_boxes,thumb);
						},scrollspeed)
	...
	function a3_portfolio_openDetailView(portfolio_boxes,thumb) {
