...
function a3_portfolio_card_item_description( $portfolio_id = 0, $echo = true ) {
	...
	$item_description = '';
	if ( $a3_portfolio_global_settings['enable_cards_description'] ) {
		...
		$portolio_desc = strip_shortcodes( strip_tags( trim( $porfolio_data->post_content ) ) );
		if ( '' != $portolio_desc ) {
			$item_description = '<div class="a3-portfolio-card-description"><div>' . strip_tags( trim( $porfolio_data->post_content ) ) . '</div></div>';
		}
